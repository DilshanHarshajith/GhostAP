...
configure_clone(){
    ...
    if [[ -n "${ARG[CLONE_SSID]:-}" || -n "${DEFAULTS[CLONE_SSID]}" ]]; then
        local target="${DEFAULTS[CLONE_SSID]}"
        log "Using specified Access Point for cloning: ${target}"
        if configure_clone_resolve_target "${target}"; then
            return 0
        fi
        error "Clone target '${target}' was not found in range on ${DEFAULTS[INTERFACE]}." \
